...
export class EdgeDetector {
	...
	public get horizontalVisibleRects(): Rect[] {
		return this._foundRects.filter(
			(edge) =>
				edge.visibility.top.isVisible ||
				edge.visibility.bottom.isVisible
		);
	}
	...
	public get verticalVisibleRects(): Rect[] {
		return this._foundRects.filter(
			(edge) =>
				edge.visibility.right.isVisible ||
				edge.visibility.left.isVisible
		);
	}

	public get horizontalEdges(): Edge[] {
		return this._foundRects.flatMap((edge) =>
			[].concat(edge.translate.topEdge, edge.translate.bottomEdge)
		);
	}
	...
	public get verticalVisibleEdges(): Edge[] {
		return this._foundRects
			...
			.flatMap((edge) =>
				[].concat(
					edge.visibility.right.isVisible
						? edge.translate.rightEdge
						: [],
					edge.visibility.left.isVisible
						? edge.translate.leftEdge
						: []
				)
			);
	}
	...
	caclulateEdges(node: HTMLElement, level: number = 0) {
		...
		if (node.parentNode) {
			...
			if (
				Math.abs(nodeRect.width - parentRect.width) < 10 &&
				Math.abs(nodeRect.height - parentRect.height) < 10
			) {
				addNode = false;
			}
		}
		if (addNode) {
			...
			if (
				...
				window.getComputedStyle(node).backgroundColor !==
					window.getComputedStyle(node.parentNode as HTMLElement)
						.backgroundColor
			) {
